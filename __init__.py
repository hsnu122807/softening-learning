...
for k in range(1, data_size+1):
    ...
    if alpha > beta:
        ...
        # tau_in_each_hidden_node = tau_array  # 因配合 pruning 需修改下次執行的 tau , 不能每次 assign 原 value, 因 66 行使用 tau_in_each_hidden_node
    ...
    if hidden_node_amount > 1:  # equals to the number of hidden nodes
        ...
        for remove_index in range(hidden_node_amount):
            ...
            hidden_thresholds_val, exam_hidden_thresholds_val = sess.run([hidden_thresholds, exam_hidden_thresholds])
